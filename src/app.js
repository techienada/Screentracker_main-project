...
import { predictRiskWithModel, trainRiskModel } from "./mlModel.js";
import { predictDeepTrend, trainDeepTrendModel } from "./dlModel.js";
import { deleteUserFromApi, fetchDashboard, loginAdmin, resetApiData, saveActivityToApi, saveUserToApi } from "./apiClient.js";
import loginIllustration from "./assets/wellbeing-login.svg";
import analyticsIllustration from "./assets/analytics-report.svg";
...
function userAvatarMarkup(user, size = "md") {
  const [start, end] = getAvatarPalette(user.name || `${user.id}`);
  return `
    <div class="avatar avatar-${size}" style="background:linear-gradient(135deg, ${start}, ${end});">
      <span>${getInitials(user.name)}</span>
    </div>
  `;
}

function imageCardMarkup(src, alt, className = "") {
  return `
...
function getMlRiskPrediction(user) {
  return predictRiskWithModel(user, state.riskModel);
}

function getDlTrendPrediction(user) {
  return predictDeepTrend(user, state.deepTrendModel);
}
...
function loginScreen() {
  ...
  app.innerHTML = `
    ...
    <main class="grid grid-login">
      <section class="hero-card hero-grid">
        ...
        ${imageCardMarkup(loginIllustration, "Digital wellbeing login illustration", "hero-image-card")}
      </section>
  ...
  document.getElementById("login-form").addEventListener("submit", async (event) => {
    ...
    try {
      ...
      authenticated = email === state.admin.email.toLowerCase() && password === state.admin.password;
    }
    if (authenticated) {
      setSession(true);
...
function buildOverviewScreen() {
  ...
  return `
    <main class="grid">
      ...
      <section class="panel panel-violet visual-panel">
        <div class="panel-head">
          <div>
            <h3>Insight preview</h3>
            <p>How analytics are presented</p>
          </div>
        </div>
        ${imageCardMarkup(analyticsIllustration, "Analytics report preview", "report-image-card")}
      </section>
...
function analysisGraph(user, dlPrediction) {
  const trend = (user.weeklyTrend || []).map(Number);
  const points = [...trend, dlPrediction.predictedScore];
  const max = Math.max(...points, 100);
  const min = Math.min(...points, 35);
  ...
  const linePoints = points.map((value, index) => `${(index * xStep).toFixed(2)},${yFor(value).toFixed(2)}`).join(" ");
  const dotMarkup = points.map((value, index) => {
    const isForecast = index === points.length - 1;
    return `
      <div
        class="analysis-dot ${isForecast ? "analysis-dot-forecast" : ""}"
        style="left:calc(${(index * xStep).toFixed(2)}% - 7px); top:calc(${yFor(value).toFixed(2)}% - 7px);"
        title="${isForecast ? "Forecast" : "Observed"} ${value}"
      ></div>
    `;
  }).join("");
  ...
  return `
    <div class="analysis-graph-card">
      <div class="analysis-graph-head">
        <div>
          <span class="report-label">Trend graph</span>
          <strong>Observed vs forecast</strong>
        </div>
        <div class="analysis-legend">
          <span><i class="legend-dot"></i>Observed</span>
          <span><i class="legend-dot legend-dot-forecast"></i>Forecast</span>
        </div>
      </div>
      <div class="analysis-graph">
        <svg viewBox="0 0 100 100" preserveAspectRatio="none" class="analysis-line">
...
function reportSummary(user) {
  const behavior = buildBehaviorAnalytics(user);
  const trend = getBehaviorTrend(user);
  const mlPrediction = getMlRiskPrediction(user);
  const dlPrediction = getDlTrendPrediction(user);
  ...
  return `
    <div class="report-grid">
      ...
      <div class="report-card"><span class="report-label">ML risk prediction</span><strong>${mlPrediction.predictedLabel} (${mlPrediction.confidence}%)</strong><p>Classifier trained on ${mlPrediction.trainedOn || state.users.length} profiles.</p></div>
      <div class="report-card"><span class="report-label">DL next trend</span><strong>${dlPrediction.predictedScore}/100</strong><p>${dlPrediction.direction} expected next cycle with ${dlPrediction.confidence}% confidence.</p></div>
        ...
        ` : state.activeUserTab === "analysis" ? `
          <section class="cards split" style="margin-top:24px;">
            <div class="panel panel-violet">
              ...
              ${analysisGraph(user, dlPrediction)}
              <div class="analysis-grid">
                ...
                <div class="analysis-card"><span class="report-label">ML predicted risk</span><strong>${mlPrediction.predictedLabel}</strong><p>${mlPrediction.confidence}% confidence from a locally trained classifier.</p></div>
                <div class="analysis-card"><span class="report-label">Top ML drivers</span><strong>${mlPrediction.drivers[0]?.label || "Behavior pattern"}</strong><p>${mlPrediction.drivers.map((item) => `${item.label} ${item.direction} risk`).join(", ")}.</p></div>
                <div class="analysis-card"><span class="report-label">Class probabilities</span><strong>Low ${mlPrediction.probabilities.Low}% | Moderate ${mlPrediction.probabilities.Moderate}% | High ${mlPrediction.probabilities.High}%</strong><p>Model size: ${mlPrediction.trainedOn || state.users.length} profiles.</p></div>
                <div class="analysis-card"><span class="report-label">DL next score</span><strong>${dlPrediction.predictedScore}/100</strong><p>Deep trend model expects a ${dlPrediction.direction.toLowerCase()} move next cycle.</p></div>
                <div class="analysis-card"><span class="report-label">DL confidence</span><strong>${dlPrediction.confidence}%</strong><p>Sequence model trained on ${dlPrediction.trainedOn || state.users.length} trend windows.</p></div>
                <div class="analysis-card"><span class="report-label">Predicted change</span><strong>${dlPrediction.change > 0 ? "+" : ""}${dlPrediction.change}</strong><p>Next-score forecast against the current weekly baseline.</p></div>
              </div>
            </div>
...
async function refreshState() {
  try {
    ...
    state.admin = dashboard.admin || seedAdmin;
    state.activity = dashboard.activity || [];
    state.riskModel = trainRiskModel(state.users);
    state.deepTrendModel = trainDeepTrendModel(state.users);
    return;
  } catch {}
