...
function getFeatureVector(user) {
...
export function trainRiskModel(users = []) {
  ...
  if (trainingUsers.length < 3 || distinctLabels.size < 2) return fallbackModel();

  const weightsByLabel = Object.fromEntries(LABELS.map((label) => [label, new Array(FEATURE_CONFIG.length + 1).fill(0)]));
  for (let epoch = 0; epoch < 220; epoch += 1) {
    for (const user of trainingUsers) {
      const features = [1, ...getFeatureVector(user)];
      for (const label of LABELS) {
        ...
        for (let index = 0; index < weights.length; index += 1) {
          const penalty = index === 0 ? 0 : 0.002 * weights[index];
          weights[index] -= 0.24 * ((error * features[index]) + penalty);
        }
      }
    }
  }
  return { method: "logistic-ovr", trainedOn: trainingUsers.length, labels: LABELS, featureConfig: FEATURE_CONFIG, weightsByLabel };
}

export function predictRisk(user, model) {
  const activeModel = model || fallbackModel();
  const featureVector = getFeatureVector(user);
  const features = [1, ...featureVector];
  const rawScores = Object.fromEntries(activeModel.labels.map((label) => [label, sigmoid(dot(activeModel.weightsByLabel[label], features))]));
  ...
  return {
    ...
    probabilities: Object.fromEntries(LABELS.map((label) => [label, Math.round((probabilities[label] || 0) * 100)])),
    drivers,
  };
}
